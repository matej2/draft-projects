...
For development, you can run package directly without installing the package. First, setup pipenv for the project, then run:

    pipenv run python -m pybroadcast.cli [action] [--port|-p <port>]

    or    

    docker build -t pybroadcast:dev . && docker run --rm pybroadcast:dev [action]
